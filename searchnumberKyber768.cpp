...
int main()
{
	...
	int mula;
	int i,j,k,t;
	int array3[32]={0};//find out the differnt bit strings and record how many (a,b) paris correspond to each bit string in array3
	...
	int i_output2,j_output2,k_output2,l_output2;//output the first one of the better choices
	...
	for (a = 0; a<du; a++)
	{
		for (b = 0; b<dv; b++)
		{
		   ...
		   //for a (a, b) pair, m_0's values form a 5-bit string  when S_t takes 5 different values, 
           ...
           //convert the bit strings into integers for the convenience of finding the same bit string 
			sum=0;
			mula=1;
			for(st=0;st<pv;st++)//the length of the bit string is 5
			{
	...
	j=0;
	for(i=0;i<mpv;i++)//since the length of the bit string is 5, it has 2^5 possibles at most
	{
		if (array3[i]!=0)
		{
			array4[j]=i;
	...
	for (i=0;i<count;i++)//count=10
		for(j=i+1;j<count;j++)//count=10
			for(k=j+1;k<count;k++)//count=10	
				...
				cout<<"there are "<<number_better<<" better choice,";
				cout<<"in order to output the better choice above,"<<endl;
				flag_i=true; 
				flag_j=true;
				flag_k=true;
				flag_l=true;
