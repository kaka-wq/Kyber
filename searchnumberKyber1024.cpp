...
int main()
{
	...
	int array4[32]={0};//record the differnt bit strings in array4 and transform the bit string into integer and record it in array4
	...
	for (a = 0; a<du; a++)
	{
		for (b = 0; b<dv; b++)
		{
		   ...
		   //for a (a, b) pair, m_0's values constitutes a 5-bit string when S_t takes 5 different values 
           ...
           //convert the bit strings into integers for the convenience of finding the same bit string 
			sum=0;
			mula=1;
			for(st=0;st<pv;st++)//S_t has 5 possible values
			{
	...
	for (i=0;i<count;i++)//count=10
		for(j=i+1;j<count;j++)//count=10
			for(k=j+1;k<count;k++)//count=10	
				...
				{
					flag=true;
					for(t=0;t<pv;t++)//s_t has 5 possible values
					{
				...
				cout<<"in order to output the first one,"<<endl;
			    for (a = 0; a<du; a++)
				{
					for (b = 0; b<dv; b++)
					{
				...
				cout<<"there are "<<number_better<<" better choices,";
