...
int main()
{
	...
	int a,b;//a,b
	int st;//S_t
	int pv=7;//7 possible values of S_t,namely {-3,-2,-1,0,1,2,3}
	int mpv=128;//2^pv
	...
	int mula;
	int i,j,k,t;
	int array3[128]={0};//2^pv,find out the differnt bit strings and record how many (a,b) paris correspond to each bit string in array3
	int array4[128]={0};//2^pv,record the differnt bit strings in array4 and transform the bit string into integer and record it in array4
	int count=0;//the number of the differnt bit strings,count=10

	int temp;
	int array5[7][17]={0};//5 is the possible values of S_t and 17 is the number of the differnt bit strings
	...
	for (a = 0; a<du; a++)
	{
		...
		{
			flag=true;
			for(st=0;st<pv;st++)//S_t in {-3,-2,-1,0,1,2,3}
			{	
	...
	for (j=0;j<count;j++)//count=17
	{
		...
		for(i=0;i<pv;i++)//s_t has 7 possible values
		{
			cout<<array5[i][j];
		}
		cout<<endl;
   ...
   //We choose as few columns as possible from this matrix, so that each row of the selected matrix is different 
	//Since S_t has 7 possible values, at least 4 columns are needed ,but there are no 3 columns that meet the conditions
	//In particualr, we choose 4 columns
	for (i=0;i<count;i++)//count=17
		for(j=i+1;j<count;j++)//count=17
			for(k=j+1;k<count;k++)//count=17	
				for(l=k+1;l<count;l++)//count=17	
				{
					flag=true;
					for(t=0;t<pv;t++)//7 possible values
					{
					...
					if(flag)
					{
						...
						{ 
							...
							controloutput++;
							for(g=0;g<7;g++)//7 possible values
							{
                       ...
                       /*
						if (array6[2][0]!=array6[0][0] && array6[2][0]!=array6[1][0]  && array6[2][0]!=array6[3][0] && array6[2][0]!=array6[4][0])
						{
							...
							{
								controloutput++;
								for(g=0;g<7;g++)//5 possible values
								{
